...
```bash
npx @58pic/mcp-server
```

服务默认启动在 `http://localhost:3456`，将客户端指向 `http://localhost:3456/mcp`。
...
|---|---|---|
| `PORT` | `3456` | 监听端口 |
| `PUBLIC_URL` | `https://mcp.58pic.com` | 对外地址（影响 OAuth 回调 URL） |
| `UPSTREAM_BASE` | `https://ai.58pic.com/api` | 上游 API 地址 |
